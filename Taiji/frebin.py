...
https://github.com/benjaminpope/pysco
...
def rebin(a, new_shape):
    ...
    if m<M:
        return a.reshape((m, int(M/m),nn,int(N/nn))).mean(3).mean(1)
    else:
        return np.repeat(np.repeat(a, m/M, axis=0), nn/N, axis=1)
...
#     new_im = np.zeros((y_newsize, x_newsize), dtype=float)
