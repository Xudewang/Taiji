...
def hsc_psf_tool(rerun_field, data_type_command, coor_ra, coor_dec,
    ...
    if os.path.exists(check_psf_file_name_Iband):
        print('At least HSC-I band PSF exists, we stop this download.'.format(
            filter_band))
...
def hsc_query_tool(sql_file, catalog_file, dr_type, data_path, code_path):
    ...
    if system_use == 'linux':
        process_sql = subprocess.Popen([
            "python /home/dewang/Taiji/hsc_pdr3/hscReleaseQuery/hscReleaseQuery.py --user='dwxu' --release-version={} --nomail --delete-job --skip-syntax-check {} --format csv > {}"
            .format(dr_type, sql_file, catalog_file)
        ],
                                       shell=True)
    elif system_use == 'darwin':
        process_sql = subprocess.Popen([
            "python /Users/xu/Astronomy/Taiji/hsc_pdr3/hscReleaseQuery/hscReleaseQuery.py --user='dwxu' --release-version={} --nomail --delete-job --skip-syntax-check {} --format csv > {}"
            .format(dr_type, sql_file, catalog_file)
        ],
                                       shell=True)
